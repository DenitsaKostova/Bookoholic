...
class Validations:
    ...
    def is_valid_isbn(cls, isbn):
        ...
        if len(formated_isbn) == 10:
            try:
                checksum = str(11 - (sum(int(formated_isbn[i]) * (10 - i)
                               for i
                               in range(0, 9)) % 11))
            except ValueError:
                return False
            if checksum == '11':
                ...
                checksum = 'X'
            if (checksum == formated_isbn[9] and
               Validations.equal(VALID_ISBN, formated_isbn)):
                return True
            else:
                return False
            ...
            try:
                checksum = str(10 - (sum(int(formated_isbn[i])
                               + int(formated_isbn[i+1])*3
                               for i
                               in range(0, 11, 2)) % 10))
            except ValueError:
                return False
            if checksum == '10':
                checksum = '0'
            if (checksum == formated_isbn[12] and
               Validations.equal(VALID_ISBN, formated_isbn)):
                return True
            else:
                return False
